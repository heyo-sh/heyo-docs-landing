...
Configuration accepts strings, but the runtime only normalizes the semantic
names listed above. A new semantic name requires a Heyo Docs source change:
extend `SemanticIcon` and its alias map, then add the matching icon component
to each application mapping. This is intentional; a random configuration
string cannot cause an arbitrary icon package to be included in the client
bundle.

For one-off visual decoration inside an MDX article, use an ordinary MDX or
React component instead of expanding the global semantic icon contract.

## Check icon-only controls

After changing a mapping, test the mobile menu, search, section chevrons,
theme toggle, code-copy button, and footer links. Semantic mappings give those
controls consistent icons, while their accessible labels are supplied by the
UI itself.
