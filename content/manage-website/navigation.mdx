...
use `{ title, src, icon? }` for a custom sidebar link.
...
article navigation. Add an optional `icon` to display a semantic sidebar icon.
They are useful for an admin console, status page, or a separate support
destination. Use a regular path such as `/changelog` for a same-site
destination.
