...
uses regular links; the generated template adds client-side navigation,
icons, and a persisted colour-mode control.
...
Run `bun run dev` and open `/`. For a hybrid deployment with the optional
request proxy, or for generated OpenAPI pages, use the complete Astro template
from `create-heyo-docs`; its adapter and resource routes handle that boundary.
